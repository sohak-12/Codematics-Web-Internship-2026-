...
const fetchFromTMDB = async (endpoint, params = {}) => {
  const response = await axios.get(API_BASE_URL + endpoint, {
    params: Object.assign({ api_key: API_KEY, language: 'en-US', include_adult: false }, params),
  });
...
const filterSafe = (results) => {
  ...
  return results.filter(function(item) { return !item.adult; });
};
...
app.get('/api/movie-genres', async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching genres' }); }
});
...
app.get('/api/movies/genre/:genreId', withPage, async (req, res) => {
  ...
  } catch(e) { res.status(500).json({ error: 'Error fetching movies by genre' }); }
});
...
app.get('/api/upcoming-movies', withPage, async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching upcoming movies' }); }
});
...
app.get('/api/popular-movies', withPage, async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching popular movies' }); }
});
...
app.get('/api/trending-movies', withPage, async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching trending movies' }); }
});
...
app.get('/api/now-playing-movies', withPage, async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching now playing movies' }); }
});
...
app.get('/api/top-rated-movies', withPage, async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching top rated movies' }); }
});
...
app.get('/api/movie/:id', async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching movie details' }); }
});

app.get('/api/movie/:id/credits', async (req, res) => {
  try { const data = await fetchFromTMDB('/movie/' + req.params.id + '/credits'); res.json(data); }
  catch(e) { res.status(500).json({ error: 'Error fetching movie credits' }); }
});
...
app.get('/api/tv-trailer/:id', async (req, res) => {
  try {
    ...
    const filtered = (data.results || []).filter(function(v) { return v.site === 'YouTube' && v.official && ['Trailer', 'Teaser'].indexOf(v.type) !== -1; });
    res.json(filtered);
  } catch(e) { res.json([]); }
});
...
app.get('/api/tv-genres', async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching TV genres' }); }
});
...
app.get('/api/tvshows/genre/:genreId', withPage, async (req, res) => {
  try {
    ...
    res.json(filterSafe(data.results));
  } catch(e) { res.status(500).json({ error: 'Error fetching TV shows by genre' }); }
});
...
app.get('/api/trending-tv', withPage, async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching trending TV shows' }); }
});
...
app.get('/api/tvshows/airing-today', withPage, async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching TV shows airing today' }); }
});
...
app.get('/api/tvshows/on-the-air', withPage, async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching TV shows on the air' }); }
});
...
app.get('/api/tvshows/popular', withPage, async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching popular TV shows' }); }
});
...
app.get('/api/tvshows/top-rated', withPage, async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching top-rated TV shows' }); }
});
...
app.get('/api/tv/:id', async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching TV show data' }); }
});
...
app.get('/api/tv/:id/aggregate_credits', async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching TV show credits' }); }
});
...
app.get('/api/tv/:id/external_ids', async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching external IDs' }); }
});
...
app.get('/api/popular-people', withPage, async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching popular people' }); }
});
...
app.get('/api/person/:id', async (req, res) => {
  try {
    ...
    res.json(data);
  } catch(e) { res.status(500).json({ error: 'Error fetching person details' }); }
});
...
app.get('/api/person/:id/tv-credits', async (req, res) => {
  try {
    ...
    if (data.crew) data.crew = filterSafe(data.crew);
    res.json(data);
  } catch(e) { res.status(500).json({ error: 'Error fetching TV credits' }); }
});

app.get('/api/discover/movie', withPage, async (req, res) => {
  try {
    var params = { page: req.query.page, include_adult: false, include_video: false };
  ...
  } catch(e) { res.status(500).json({ error: 'Error discovering movies' }); }
});

app.get('/api/discover/tv', withPage, async (req, res) => {
  try {
    var params = { page: req.query.page, include_adult: false };
    if (req.query.sort_by) params.sort_by = req.query.sort_by;
    if (req.query.with_genres) params.with_genres = req.query.with_genres;
    ...
    res.json({ results: filterSafe(data.results), total_pages: data.total_pages });
  } catch(e) { res.status(500).json({ error: 'Error discovering TV shows' }); }
});
...
app.get('/api/search', async (req, res) => {
  try {
    ...
    res.json(data);
  } catch(e) { res.status(500).json({ error: 'Error searching' }); }
});
...
app.get('/api/trending-all', withPage, async (req, res) => {
  ...
  catch(e) { res.status(500).json({ error: 'Error fetching trending all' }); }
});

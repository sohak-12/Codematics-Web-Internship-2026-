...
import { getMovieGenres, discoverMovies } from '../../api/tmdbClient';
...
const FilmsIndex = () => {
  ...
  useEffect(() => {
    getMovieGenres().then(g => setGenres(g || []));
  }, []);

  const fetchMovies = useCallback(async (pg = 1, reset = false) => {
  ...
  const loadMore = () => { if (!paginating && hasMore) fetchMovies(page + 1); };
      ...
      <Section>
        ...
        <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
          <div className="d-flex align-items-center gap-2">
            <MdMovieFilter className="text-primary fs-4" />
            <p className="h5 text m-0 section-heading">Discover Movies</p>
          </div>
          <div className="d-flex gap-2 align-items-center">
            <button
              ...
              style={{
                background: showFilters ? 'var(--accent-soft)' : 'var(--bg-primary)',
                border: `1px solid ${showFilters ? 'var(--accent)' : 'var(--divider)'}`,
                color: showFilters ? 'var(--accent)' : 'var(--text-secondary)',
                borderRadius: 10, padding: '6px 14px', fontWeight: 600, fontSize: '0.8rem',
              }}
            >
              <BiFilterAlt /> Filters
            </button>
            <div className="d-flex align-items-center gap-1" style={{ ...selectStyle, padding: 0, border: 'none', background: 'none', minWidth: 'auto' }}>
        ...
        <div className="d-flex gap-2 mb-3 overflow-auto scrollbar-custom pb-1" style={{ scrollbarWidth: 'none' }}>
          <button
            ...
            style={{
              background: !activeGenre ? 'linear-gradient(135deg, var(--accent), var(--accent-hover))' : 'var(--bg-primary)',
              color: !activeGenre ? '#fff' : 'var(--text-secondary)',
              border: `1px solid ${!activeGenre ? 'var(--accent)' : 'var(--divider)'}`,
              borderRadius: 50, padding: '6px 16px', fontWeight: 600, fontSize: '0.78rem',
              whiteSpace: 'nowrap',
            }}
          >
          ...
          {genres.map(g => (
            <button
              key={g.id}
              onClick={() => setActiveGenre(String(g.id) === activeGenre ? '' : String(g.id))}
              className="btn btn-sm flex-shrink-0"
              style={{
                background: String(g.id) === activeGenre ? 'linear-gradient(135deg, var(--accent), var(--accent-hover))' : 'var(--bg-primary)',
                color: String(g.id) === activeGenre ? '#fff' : 'var(--text-secondary)',
                border: `1px solid ${String(g.id) === activeGenre ? 'var(--accent)' : 'var(--divider)'}`,
                borderRadius: 50, padding: '6px 16px', fontWeight: 600, fontSize: '0.78rem',
        ...
        {showFilters && (
          <div className="d-flex gap-2 mb-3 flex-wrap" style={{ animation: 'pageReveal 0.3s ease-out' }}>
            ...
            {(year || rating || lang) && (
              <button
                onClick={() => { setYear(''); setRating(''); setLang(''); }}
        ...
        {loading ? (
          ...
          <div className="text-center py-5">
            <p className="text-secondary">No movies found with these filters. Try adjusting your selection.</p>
          </div>
        ) : (
          <>
                  ...
                  <MediaCard>
                    <Link to={`/movies/${item.id}`}>
                      <LazyLoad height={200} offset={100} placeholder={<img src="/default-poster.svg" alt="loading" className="card-img-top" />}>
                        <img src={item.poster_path ? `https://image.tmdb.org/t/p/w500${item.poster_path}` : '/default-poster.svg'} className="card-img-top" alt={item.title || '-'} />
                      </LazyLoad>
                    </Link>
                    <div className="card-body">
                      <div className="d-flex justify-content-between align-items-center gap-3 mb-2">
                        <small className="text-secondary">{extractYear(item.release_date)}</small>
                      ...
                      <div className="title-wrapper"><p className="card-title text m-0">{item.title}</p></div>
                    </div>
                  </MediaCard>
                </div>
              ))}
